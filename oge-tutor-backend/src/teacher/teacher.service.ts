...
import { logDb } from '../common/app-logger';
...
export class TeacherService {
  ...
  async updateProfile(user: AuthUser, patch: any) {
    ...
    logDb('update teacher profile', { teacherId });
    return teacher;
  }
  ...
  async updateAccount(user: AuthUser, payload: any) {
    ...
    logDb('update teacher account', { teacherId, userId: teacher.userId });
  ...
  async changePassword(user: AuthUser, payload: any) {
    ...
    logDb('update teacher password', { teacherId, userId: teacher.userId });
    return this.prisma.teacherProfile.findUniqueOrThrow({ where: { id: teacherId }, include: { user: true } });
  }

  async updateNotifications(user: AuthUser, payload: any) {
    const teacherId = this.assertTeacher(user);
    const teacher = await this.prisma.teacherProfile.update({ where: { id: teacherId }, data: { settings: payload?.settings || payload || {} }, include: { user: true } });
    logDb('update teacher notifications', { teacherId });
    return teacher;
  }
}
