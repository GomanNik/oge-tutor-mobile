...
const EDITABLE_HOMEWORK_STATUSES = [HOMEWORK_STATUS.ASSIGNED, HOMEWORK_STATUS.OVERDUE, HOMEWORK_STATUS.NEEDS_REVISION];
const SUBMITTABLE_HOMEWORK_STATUSES = [HOMEWORK_STATUS.ASSIGNED, HOMEWORK_STATUS.OVERDUE, HOMEWORK_STATUS.NEEDS_REVISION];

function topicTitleFromTasks(taskNumbers: number[]): string {
...
export class HomeworksService {
  ...
  async create(user: AuthUser, payload: any) {
    ...
    const taskNumbers = parseTaskNumbers(payload.taskNumbers, 'taskNumbers');
    return this.prisma.homework.create({
      data: {
        ...
        topic: cleanText(payload.topic) || topicTitleFromTasks(taskNumbers),
        taskNumbers,
        dueAt,
  ...
  async update(user: AuthUser, homeworkId: string, patch: any) {
    ...
    if (!EDITABLE_HOMEWORK_STATUSES.includes(homework.status as any)) {
      throw conflict('Эту домашнюю работу уже нельзя редактировать.', { status: 'locked' });
    }
    if (patch.status !== undefined) {
      throw validationError('Статус ДЗ меняется только через отправку или проверку.', { status: 'use_lifecycle_endpoint' });
    }
    const data: any = {};
    ...
    if (patch.taskNumbers !== undefined) {
      data.taskNumbers = parseTaskNumbers(patch.taskNumbers, 'taskNumbers');
      if (patch.topic === undefined) data.topic = topicTitleFromTasks(data.taskNumbers);
    }
    ...
    return this.prisma.homework.update({ where: { id: homeworkId }, data });
  }

  async submit(user: AuthUser, homeworkId: string, file: Express.Multer.File, payload: any = {}) {
    const homework = await this.requireStudentHomework(user, homeworkId);
    if (!SUBMITTABLE_HOMEWORK_STATUSES.includes(homework.status as any)) {
      throw conflict('Сейчас это ДЗ нельзя отправить.', { status: 'locked' });
    }
    ...
    const solutionFile = cleanText(payload.fileTitle) || fileResource.originalName;
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.homework.update({
        where: { id: homeworkId },
        data: { status: HOMEWORK_STATUS.SUBMITTED, submittedAt: new Date(), reviewedAt: null, closedAt: null, solutionFile },
      });
    });
  }
  ...
  async review(user: AuthUser, homeworkId: string, payload: any) {
    const teacherId = this.assertTeacher(user);
    const homework = await this.requireTeacherHomework(teacherId, homeworkId);
    if (homework.status !== HOMEWORK_STATUS.SUBMITTED) {
      throw conflict('Проверять можно только сданную работу.', { status: 'invalid_status' });
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      const latest = await tx.homeworkSubmission.findFirst({ where: { homeworkId }, orderBy: { submittedAt: 'desc' } });
      if (!latest || !homework.solutionFile) {
        throw conflict('Проверять можно только работу с отправленным решением.', { submission: 'required' });
      }
      await tx.homework.update({
        where: { id: homeworkId },
        data: {
      ...
      await tx.homeworkSubmission.update({ where: { id: latest.id }, data: { reviewStatus: submissionStatus, reviewedAt, teacherComment: comment } });
