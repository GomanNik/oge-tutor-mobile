...
export class BootstrapService {
  ...
  async buildForUser(userId: string) {
    ...
    if (user.role === ROLE.TEACHER && user.teacherProfile) {
      const data = await this.buildForTeacher(user.teacherProfile.id);
      logDomain('bootstrap.loaded', { role: user.role, profileId: user.teacherProfile.id });
      return data;
    }
    if (user.role === ROLE.STUDENT && user.studentProfile) {
      const data = await this.buildForStudent(user.studentProfile.id);
      logDomain('bootstrap.loaded', { role: user.role, profileId: user.studentProfile.id });
      return data;
    }
