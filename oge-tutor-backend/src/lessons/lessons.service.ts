...
import { assertLessonInterval, cleanText, parseIsoDate, parseOptionalTaskNumbers, requireText } from '../common/validation';
...
export class LessonsService {
  ...
  async assertNoConflict(studentId: string, startAt: Date, endAt: Date, lessonId?: string) {
    const conflictLesson = await this.prisma.lesson.findFirst({
      where: {
        ...
        status: { not: LESSON_STATUS.CANCELED },
        startAt: { lt: endAt },
        endAt: { gt: startAt },
      },
    });
  ...
  async create(user: AuthUser, payload: any) {
    ...
    logDb('create lesson', { id: lesson.id, teacherId, studentId });
    logDomain('lesson.created', { lessonId: lesson.id, teacherId, studentId, focusTaskNumbers: lesson.focusTaskNumbers });
    return lesson;
  }
  ...
  async update(user: AuthUser, lessonId: string, patch: any) {
    ...
    logDb('update lesson', { id: lessonId, teacherId, status: status || lesson.status });
    return updated;
  }
  ...
  async complete(user: AuthUser, lessonId: string, payload: any) {
    const teacherId = this.assertTeacher(user);
    const lesson = await this.assertLessonBelongsToTeacher(teacherId, lessonId);
    if (![LESSON_STATUS.PLANNED, LESSON_STATUS.RESCHEDULED].includes(lesson.status as any)) {
      throw validationError('Завершить можно только запланированный или перенесённый урок.', { status: 'invalid_transition' });
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      const completedLesson = await tx.lesson.update({
        where: { id: lessonId },
        data: {
      ...
      logDb('update lesson complete', { id: lessonId, teacherId, studentId: lesson.studentId });
      logDomain('lesson.completed', { lessonId, studentId: lesson.studentId, teacherId, focusTaskNumbers });
      ...
      for (const taskNumber of focusTaskNumbers) {
        ...
        logDb('update progress', { studentId: lesson.studentId, taskNumber, coverageStatus: PROGRESS_COVERAGE_STATUS.ASSESSMENT_NEEDED });
        logDomain('progress.assessment_needed', { studentId: lesson.studentId, taskNumber, lessonId, historyId: history.id });
        ...
        logDomain('notification.created', { notificationId: notification.id, studentId: lesson.studentId, taskNumber, lessonId });
      }
