...
export class MaterialsController {
  ...
  async add(@CurrentUser() user: AuthUser, @Body() body: AddMaterialDto) {
    const material = await this.materials.add(user, body);
    return { material, data: await this.bootstrap.buildForUser(user.id) };
  }
  ...
  async remove(@CurrentUser() user: AuthUser, @Param('topicId') topicId: string, @Param('fileId') fileId: string) {
    const removedMaterialFile = await this.materials.removeFile(user, topicId, fileId);
    return { removedMaterialFile, data: await this.bootstrap.buildForUser(user.id) };
  }
}
