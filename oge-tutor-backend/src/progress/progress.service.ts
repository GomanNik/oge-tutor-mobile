...
export class ProgressService {
  ...
  async updateTask(user: AuthUser, studentId: string, taskNumberInput: unknown, payload: any) {
    ...
    await this.prisma.progressHistory.create({ data: { studentId, taskProgressId: progress.id, taskNumber, type: 'manual_update', source: PROGRESS_SOURCE.MANUAL, coverageStatus, masteryLevel, comment: cleanText(payload.teacherComment) } });
    return progress;
  }
