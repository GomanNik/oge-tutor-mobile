...
import { Badge, Card, Header, MaterialList, RowCard } from '../shared/ui.jsx';
import { formatMaterialCount } from '../shared/formatters.js';

export function StudentMaterials({ materials, openTopic }) {
  return <><Header title="Материалы" subtitle="Материалы сгруппированы по номерам заданий ОГЭ" />{materials.map((topic) => <RowCard key={topic.id} icon={topic.taskNumber} iconTone="blue" title={topic.title} subtitle={formatMaterialCount(topic.files.length)} onClick={() => openTopic(topic.id)} />)}</>;
}
